...
const FAST_MOUNT_RUNTIME_IMPORT_SOURCES = new Set(['vue-fast-mount'])

function isFastMountRuntimeImportSource(source: string): boolean {
  if (FAST_MOUNT_RUNTIME_IMPORT_SOURCES.has(source)) {
    return true
  }

  return /^(?:\.{1,2}\/)+(?:src\/)?runtime(?:\.[cm]?[jt]s)?$/.test(source)
}
...
function rewriteFastMountCallsites(code: string): string {
...
function getPlaceholderComponentName(local: string): string {
  if (local.endsWith('Child')) {
    return 'Child'
  }
...
function createPlaceholderComponentDeclarations(
  ...
  return locals
    .map((local) => {
      ...
      const componentName = getPlaceholderComponentName(local)
      const stubTag = `${toKebabCase(componentName)}-stub`

      return `const ${local} = { name: '${componentName}'${propsPart}${emitsPart}, render() { const normalizedAttrs = Object.fromEntries(Object.entries(this.$attrs).map(([key, value]) => [key === 'datatestid' ? 'data-testid' : key.replace(/[A-Z]/g, (character) => '-' + character.toLowerCase()), value])); return h('${stubTag}', { ...normalizedAttrs, ...this.$props }) } }`
    })
    .join('\n')
}
...
function transformFastMountVueSource(code: string, keepBindings: Set<string>): string {
...
function shouldTransformVueFastMountId(id: string): boolean {
...
function getKeepBindingsFromId(id: string): Set<string> {
